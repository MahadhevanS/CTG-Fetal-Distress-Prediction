"""
Training & Stratified 5-Fold Patient-Level CV Script for Multi-Scale LSTM Encoder
=================================================================================

Phase 3 Model 5: Benchmarking MultiScaleLSTMEncoder on CTG Fetal Distress Dataset.
Conforms strictly to 5-Fold Stratified Patient-Level Cross Validation protocol.
"""
...
class CTGDataset(Dataset):
    def __init__(self, X, y, patient_ids=None):
        self.X = torch.tensor(X, dtype=torch.float32) if not torch.is_tensor(X) else X.float()
        self.y = torch.tensor(y, dtype=torch.float32) if not torch.is_tensor(y) else y.float()
...
def generate_synthetic_data(num_samples=160, num_patients=20):
    ...
    patient_ids = np.array([f"P{i % num_patients:03d}" for i in range(num_samples)])
    return X, y, patient_ids


def main():
    parser = argparse.ArgumentParser(description="Benchmarking Multi-Scale LSTM Encoder")
    ...
    parser.add_argument("--hidden_size", type=int, default=64, help="LSTM hidden size per direction")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--dry_run", action="store_true", help="Run in dry-run synthetic mode for quick testing")
    args = parser.parse_args()

    set_seed(args.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Executing Multi-Scale LSTM Benchmark on device: {device}")
    ...
    if not args.dry_run and os.path.exists(train_pt):
        ...
        X = data['X'].numpy() if torch.is_tensor(data['X']) else np.array(data['X'])
        y = data['y_primary'].numpy() if torch.is_tensor(data['y_primary']) else np.array(data['y_primary'])
        metadata = data['metadata']
        patient_ids = np.array([m[0] for m in metadata])
    else:
        if not args.dry_run:
            print(f"\n[!] WARNING: Dataset file 'train_dataset.pt' not found at '{train_pt}'.")
            print("    Falling back to synthetic dataset for verification...\n")
    ...
    for fold, (train_idx, val_idx) in enumerate(sgkf.split(X, y, groups=patient_ids), 1):
        ...
        model = MultiScaleLSTMForClassification(encoder, hidden_dim=64, dropout=0.2).to(device)
        ...
        for epoch in epoch_pbar:
            ...
            if val_metrics['auroc'] > best_val_auroc:
                ...
                ckpt_path = os.path.join(args.checkpoint_dir, f"multiscale_lstm_fold_{fold}_best.pth")
        ...
        print(f"\nFold {fold} Best | AUROC: {metrics['auroc']:.4f} | AUPRC: {metrics['auprc']:.4f} | "
              f"Sens: {metrics['recall']:.2f}% | Spec: {metrics['specificity']:.2f}% | F1: {metrics['f1']:.4f}\n")
    ...
    if not args.dry_run and os.path.exists(test_pt):
        ...
        test_X = test_data['X'].numpy() if torch.is_tensor(test_data['X']) else np.array(test_data['X'])
        test_y = test_data['y_primary'].numpy() if torch.is_tensor(test_data['y_primary']) else np.array(test_data['y_primary'])
        test_ds = CTGDataset(test_X, test_y)
    ...
    print(f"\nMulti-Scale LSTM Encoder Total Trainable Parameters: {total_params:,}")
    print("=================================================================\n")
